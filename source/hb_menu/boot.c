...
void (*callBootloaderRunTitle_2x)(u8 mediatype, u32 *argbuf, u32 argbuflength, u32 tid_low,
                                  u32 tid_high) = (void *) 0x00100010;

void (*getBestProcess_2x)(u32 sectionSizes[3], bool *requirements, int num_requirements, processEntry_s *out,
                          int out_size, int *out_len) = (void *) 0x0010000C;
...
static void launchFile_2x(void) {
    ...
    if (targetProcessId == -1)callBootloader_2x(hbFileHandle, argbuffer, argbuffer_length);
        ...
        callBootloaderRunTitle_2x(target_title.mediatype, argbuffer, argbuffer_length,
                                  target_title.title_id & 0xffffffff, (target_title.title_id >> 32) & 0xffffffff);
    ...
    if (!hbInit()) {
        ...
        if (em) {
            if (em->scanned && targetProcessId == -1) {
                ...
                int out_len = 0;
                getBestProcess_2x(em->sectionSizes, (bool *) em->servicesThatMatter, NUM_SERVICESTHATMATTER, out, 4,
                                  &out_len);
